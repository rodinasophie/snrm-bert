...
class TrainDataLoader:
    def __init__(self, queries, triples, docs_dict):
    ...
    def __load_data(self):
        ...
        self.df_triples = pd.read_csv(
            self.triples, sep="\t", index_col=False, na_filter=False, header=None
        )
        self.triples_len = self.df_triples.shape[0]
    ...
    def generate_triple_batch(self, batch_size):
        ...
        docs_1 = []
        docs_2 = []
        triples_len = self.df_triples.shape[0]
        new_offset = min(self.triple_offset + batch_size, triples_len)
        is_end = True if new_offset == triples_len else False

        for i in range(self.triple_offset, new_offset):
            qrel = self.df_triples.loc[i]  # id_query, id_doc, id_irr_doc
            queries.append(
                self.df_queries.loc[self.df_queries["id_left"] == qrel[0]][
                    "text_left"
                ].values[0]
            )
            content = self.__get_content(str(qrel[1]))
            docs_1.append(content[0])

            docs_2.append(self.__get_content(str(qrel[2]))[0])
...
class DataLoader:
    ...
    def __load_data(self):
        ...
        self.df_qrels = pd.read_csv(
            self.qrels, sep=" ", index_col=False, na_filter=False, header=None
        )
